...
def collideLineLine(l1_p1, l1_p2, l2_p1, l2_p2):
    ...
    return (
        t > 0
        and u > 0
        and t * t < line1_vec.magnitude_squared()
        and u * u < line2_vec.magnitude_squared()
    )


def colideRectLine(rect, p1, p2):
    return (
        collideLineLine(p1, p2, rect.topleft, rect.bottomleft)
        or collideLineLine(p1, p2, rect.bottomleft, rect.bottomright)
        or collideLineLine(p1, p2, rect.bottomright, rect.topright)
        or collideLineLine(p1, p2, rect.topright, rect.topleft)
    )
...
class Guard:
    ...
    def update(
        ...
        surface.blit(
            self.animator.get(), (self.rect.x - camera[0], self.rect.y - camera[1])
        )

    def detect_target(self, target_rect: pygame.Rect, camera) -> bool:
        x = self.head[0] + self.radius * math.cos(self.half_ang)
        y1 = self.head[1] + self.radius * math.sin(self.half_ang)
        y2 = self.head[1] - self.radius * math.sin(self.half_ang)
    ...
    def __init_animation(self):
        # Walk animation
        self.animator.add(
            Dir.right, State.walk, self.sprite.load_strip([0, 0, 32, 32], 3), 1
        )
        self.animator.add(
            Dir.left,
            State.walk,
            [
                ...
                for i in self.sprite.load_strip([0, 0, 32, 32], 3)
            ],
            1,
        )
        ...
        # Idle animation
        self.animator.add(
            Dir.right, State.idle, self.sprite.load_strip([0, 1, 32, 32], 2), 0.3
        )
        self.animator.add(
            Dir.left,
            State.idle,
            [
                pygame.transform.flip(i, True, False)
                for i in self.sprite.load_strip([0, 1, 32, 32], 2)
            ],
            0.3,
        )

...
class Player:
    ...
    def update(
        ...
        if not (
            self.movement[Dir.stick]
            and (self.coll_dir[Dir.left] or self.coll_dir[Dir.right])
        ):
    ...
    def __check_collision(
        self, rects: list[pygame.Rect], delta_pos: tuple[int, int], dt: float
    ):
    ...
    def __init_animation(self):
        # Idle animation
        self.animator.add(
            Dir.right, State.idle, self.sprite.load_strip([0, 0, 18, 25], 1), 1
        )
        self.animator.add(
            Dir.left,
            State.idle,
            [
                ...
                for i in self.sprite.load_strip([0, 0, 18, 25], 1)
            ],
            1,
        )
        ...
        # Walk animation
        self.animator.add(
            Dir.right, State.walk, self.sprite.load_strip([2, 0, 18, 25], 2), 1
        )
        self.animator.add(
            Dir.left,
            State.walk,
            [
                pygame.transform.flip(i, True, False)
                for i in self.sprite.load_strip([2, 0, 18, 25], 2)
            ],
            1,
        )
        ...
        self.animator.add(
            Dir.right, State.ascent, self.sprite.load_strip([1, 0, 18, 25], 1), 1
        )
        self.animator.add(
            Dir.left,
            State.ascent,
            [
                pygame.transform.flip(i, True, False)
                for i in self.sprite.load_strip([1, 0, 18, 25], 1)
            ],
            1,
        )
        ...
        # Stick animation
        self.animator.add(
            Dir.right, State.stick, self.sprite.load_strip([4, 0, 18, 25], 1), 1
        )
        self.animator.add(
            Dir.left,
            State.stick,
            [
                pygame.transform.flip(i, True, False)
                for i in self.sprite.load_strip([4, 0, 18, 25], 1)
            ],
            1,
        )
